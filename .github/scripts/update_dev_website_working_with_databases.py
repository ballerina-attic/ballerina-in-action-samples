...
def main():
    with open('working_with_databases/working_with_databases.bal', 'r') as action_obj:
        updated_bal_file = action_obj.read()
        with open('working-with-databases.md', 'a') as write_obj:
            write_obj.write("```\n")
    ...
    with open('working-with-databases.md','r') as commit_obj:
        updated_file = commit_obj.read()
        
    commit_message = 'Update Ballerina in Action samples'
    try:
        update = commit_file('components/home-page/bal-action/action-bbe/working-with-databases.md', updated_file, 'update-working-with-databases', commit_message)[0]

        if update:
            pr_title = '[Automated] Update Ballerina in Action samples'
            pr_body = 'Update Ballerina in Action samples for source code changes in `Working with databases`'
            head_branch = 'update-working-with-databases'
