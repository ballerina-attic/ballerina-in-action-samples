...
def main():
    with open('graohql_api/graphql_api.bal', 'r') as action_obj:
        updated_bal_file = action_obj.read()
        with open('graphql-api.md', 'a') as write_obj:
            write_obj.write("```\n")
    ...
    with open('graphql-api.md','r') as commit_obj:
    ...
    try:
        ...
        if update:
            ...
            pr_body = 'Update Ballerina in Action samples for source code changes in `GraphQL API`'
            head_branch = 'update-graphql-api'
